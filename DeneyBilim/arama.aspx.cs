...
namespace DeneyBilim
{
    public partial class arama : System.Web.UI.Page
    {
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            object ara = Session["ara"];

            if (ara == null)
            {
                SqlDataSource0.SelectCommand = "SELECT icerikid,icerikfotograf,icerikbaslik FROM icerik ORDER BY icerikzaman ASC";
                Label1.Text = "Aranan Kelime: *";
            }
            else if (ara != null)
            {
                SqlDataSource0.SelectCommand = "SELECT icerikid,icerikfotograf,icerikbaslik FROM icerik WHERE icerikbaslik LIKE '%" + ara + "%' OR icerikmetin LIKE '%" + ara + "%' OR icerikturu LIKE '%" + ara + "%' ORDER BY icerikzaman ASC";
                Label1.Text = "Aranan Kelime: " + ara;
            }
...
(System.Web.Configuration.WebConfigurationManager.ConnectionStrings["veritabani"].ConnectionString);
            ...
            SqlCommand db_komut = new SqlCommand("SELECT TOP 3 * FROM icerik WHERE icerikturu LIKE '%Haber%' ORDER BY NEWID()", db_baglanti);
